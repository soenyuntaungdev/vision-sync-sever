...
def audit_master_coverage(data_yaml_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        names = _normalize_names(data.get("names", {}))
        if not names:
            return {"ok": False, "message": "data.yaml ထဲတွင် names မရှိပါ"}
        ...
        if dataset_root and not os.path.isabs(dataset_root):
            root_dir = os.path.normpath(os.path.join(yaml_dir, dataset_root))
        elif dataset_root:
            root_dir = dataset_root
        else:
            root_dir = yaml_dir

        counts: Dict[int, int] = {cid: 0 for cid in names.keys()}

        lbl_dirs = []
        for split in ["train", "val", "valid", "test"]:
            p1 = os.path.join(root_dir, split, "labels")
            p2 = os.path.join(root_dir, "labels", split)
            if os.path.isdir(p1):
                lbl_dirs.append(p1)
            if os.path.isdir(p2):
                lbl_dirs.append(p2)

        for d in set(lbl_dirs):
        ...
        class_counts = {names[cid]: count for cid, count in counts.items()}
        empty_classes = [names[cid] for cid, count in counts.items() if count == 0]
        ...
        return {
            ...
            "message": f"Audited {len(names)} classes, found {len(empty_classes)} empty classes."
        }
...
def _shift_label_file(label_path: str, out_path: str, target_class_id: int,
                      source_nc: int, valid_source_ids: Optional[set] = None,
                      id_map: Optional[Dict[int, int]] = None) -> Tuple[int, int]:
    copied = 0
    dropped = 0
...
def merge_dataset_into_master(
    ...
    candidate_ids = sorted(src_names.keys()) if src_names else list(range(max(1, src_nc)))
    if source_class_ids:
        wanted = set(source_class_ids)
        candidate_ids = [i for i in candidate_ids if i in wanted]
        valid_source_ids = set(candidate_ids)
    ...
    for src_split, dst_split, src_img_dir, src_lbl_dir in splits:
        ...
        for fname in sorted(os.listdir(src_img_dir)):
            ...
            if os.path.isfile(src_lbl):
                c, d = _shift_label_file(src_lbl, dst_lbl, master_class_id, src_nc, valid_source_ids, id_map)
                total_labels += c
                total_dropped += d
            else:
...
def _cli() -> None:
    ...
    p3.add_argument("--freeze", type=int, default=10,
                    help="Pre-trained backbone layers freeze ပြုလုပ်မည့် အရေအတွက် (0 = မခဲ)")
